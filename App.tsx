...
const App: React.FC = () => {
  ...
  const effectiveInventory = useMemo(() => inventory.filter(item => {
      ...
      if (item.currentQuantity <= 0.001) return false; 
      // 家具为0的商品（APP赠送商品）不计入总数
      const price = item.history && item.history[0] ? item.history[0].unitPrice : 0;
  ...
  const handleRecipeCook = useCallback((recipe: Recipe) => {
    setInventory(prevInventory => 
      prevInventory.map(item => {
        ...
        if (ingredient) {
            const qtyToSub = Number(ingredient.quantityToConsume);
            return { ...item, currentQuantity: Math.max(0, item.currentQuantity - qtyToSub) };
        }
        return item;
      })
    ); 
    const msg = targetLang === 'zh-CN' ? `已按用量扣除食材，烹饪了 ${recipe.name}。` : `Cooked ${recipe.name}, deducted ingredients from stock.`;
    setButlerMessage(msg);
  }, [targetLang]); 
